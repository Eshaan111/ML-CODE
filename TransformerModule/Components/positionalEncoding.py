...
def create_pos_tensor(start_T, end_T,embed_dim, device = 'cpu'):
    ...
    embeddings = torch.zeros(size=(1, seq_len, embed_dim)).to(device)
...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, input, return_added = False):
        ...
        B_p,T_p,_ = self.positional_embeds.shape
        ...
        if T_B > T_p :
            with torch.no_grad():
                ...
                new_embed_tensor = torch.cat([self.positional_embeds, new_embeds], dim=1)
